 ...
 * 分为估算阶段和正式统计阶段，并使用 ForkJoinPool 进行多线程加速。
 */
public class CountDirFilesTask extends Task<Map<String, Long>> {
    ...
    public CountDirFilesTask(List<File> files, long timeoutMinutes, int estimationSampleSize) {
        ...
        this.fileProcessingPool = new ForkJoinPool(parallelism, ForkJoinPool.defaultForkJoinWorkerThreadFactory, null,
                true);
    }

    private java.util.regex.Pattern searchPattern;

    public void setFilterPattern(String filterPattern) {
        this.filterPattern = filterPattern;
        if (filterPattern != null && !filterPattern.isEmpty()) {
            this.searchPattern = java.util.regex.Pattern.compile(filterPattern);
        } else {
    ...
    protected Map<String, Long> call() throws Exception {
        try {
            ...
            if (searchPattern != null) {
                ...
                log.info("Starting parallel count (no filter)");
                task = fileProcessingPool.submit(new DirectoryProcessingAction(files));
            }
        ...
        try {
            ...
            for (File rootFile : files) {
                ...
                if (rootFile.isDirectory()) {
                    Files.walkFileTree(rootFile.toPath(), EnumSet.noneOf(FileVisitOption.class), MAX_DEPTH,
                            new SimpleFileVisitor<>() {
                                int visited = 0;

                                @Override
                                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                                    if (sampleCount.incrementAndGet() >= estimationSampleSize)
                ...
                if (sampleCount.get() >= estimationSampleSize)
                    break;
            }
    ...
    private class SearchingAction extends RecursiveAction {
        private final List<File> targets;

        public SearchingAction(List<File> targets) {
        ...
        protected void compute() {
            ...
            for (File file : targets) {
                ...
                if (searchPattern != null && searchPattern.matcher(file.getName()).matches()) {
                    matches = true;
                }

                if (matches) {
                    actions.add(new DirectoryProcessingAction(Collections.singletonList(file)));
                } else {
                    if (file.isDirectory()) {
                        File[] subs = file.listFiles();
                        if (subs != null && subs.length > 0) {
                            actions.add(new SearchingAction(Arrays.asList(subs)));
                        }
                    }
                }
            }
