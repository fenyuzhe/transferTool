...
import com.protoss.toolkit.entity.DirEntry;
import com.protoss.toolkit.framework.StageAware;
import com.protoss.toolkit.thread.TransferTask;
import com.protoss.toolkit.util.TransferFileUtil;
...
public class TransferToolController implements Initializable, StageAware {
    ...
    private void initializeControls() {
        ...
        dp_start.disabledProperty().addListener((obs, oldVal, nowDisabled) -> Platform
                .runLater(() -> setDatePickerEditorStyle(dp_start, nowDisabled)));
        dp_end.disabledProperty().addListener((obs, oldVal, nowDisabled) -> Platform
    ...
    private WaitingController showWaitingDialog(String message) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(
                    getClass().getResource("/com/protoss/toolkit/fxml/wating-view.fxml"));
            fxmlLoader.load();
    ...
    private void startSchedule() {
        ...
        long initialDelay = parseInitialDelay(txt_startTime.getText());
        if (initialDelay < 0) {
            return;
        }
        ...
        scheduledTask = scheduler.scheduleAtFixedRate(
                () -> runScheduledTransferIfIdle(scheduledConfig),
                initialDelay,
                interval * 60 * 1000,
                java.util.concurrent.TimeUnit.MILLISECONDS);
    }

    private long parseInitialDelay(String startTimeStr) {
        ...
        try {
            ...
            logToSched("任务将在 " + cn.hutool.core.date.DateUtil.format(target.getTime(), "yyyy-MM-dd HH:mm:ss") + " 启动");
    ...
    private void runScheduledTransfer(ScheduledTransferConfig config) {
        ...
        try {
            File root = new File(config.sourcePath());
            if (!root.exists()) {
                logToSched("错误: 源路径不存在 " + config.sourcePath());
                return;
            }

            List<File> roots = new ArrayList<>();
            if (config.daysAgo() > 0) {
                Date targetDate = cn.hutool.core.date.DateUtil.offsetDay(new Date(), -config.daysAgo());
                String targetDateStr = cn.hutool.core.date.DateUtil.format(targetDate, "yyyyMMdd");
                logToSched("筛选日期早于或等于 " + targetDateStr + " 的文件夹 (N=" + config.daysAgo() + ")");

                List<DirEntry> list = new ArrayList<>();
                java.time.LocalDate endDate = java.time.LocalDate.now().minusDays(config.daysAgo());
